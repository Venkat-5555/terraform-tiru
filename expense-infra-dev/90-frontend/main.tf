...
resource "null_resource" "frontend" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo sh /tmp/frontend.sh ${var.frontend_tags.component} ${var.enviornment}"
    ]
  }
}
...
resource "aws_lb_listener_rule" "frontend" {
  listener_arn = local.web_alb_listner_arn
  priority     = 100 #low priority evaluate first
 ...
 condition {
    host_header {
      values = ["expense-${var.enviornment}.${var.zone_name}"]
    }
 }
